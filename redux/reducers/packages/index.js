...
export const reducer = (
    ...
    switch (type) {
        ...
        case UPDATE_PACKAGE_SUCCESS:
            return { ...state, updateItem: {}, errorUpdate: false, updateLaoding: false };
...
export const updatePackageSuccess = (updatedPackage) =>
    ({ type: UPDATE_PACKAGE_SUCCESS, payload: updatedPackage });

export const updatePackageFail = (error) =>
    ({ type: UPDATE_PACKAGE_FAIL, payload: error });
...
function* sendFromTransitSaga({ payload }) {
    ...
    if (
        transitlength > 0 &&
        !item.transit[transitlength - 1].date
    ) {
...
function* acceptPackageSaga({ payload }) {
    ...
    if (isTransit) {
        if (updateItem.transit?.length) {
            const transit = updateItem.transit.slice();
